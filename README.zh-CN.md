...
这会在右侧边栏打开一个聊天面板。用自然语言提问，插件会基于你的 Wiki 作答：先读取索引，Wiki 较大时再让模型从索引中挑出相关页面、只钻取这些页面（索引优先检索）。回复以 Markdown 渲染；你可以**复制**回复，或用**保存到 Wiki**把有价值的答案经审阅式变更计划回填成一个页面（这次问答也会记录到 log 中），让探索随时间复利。
